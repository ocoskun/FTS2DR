...
public class SpireFitsIO extends ca.uol.aig.fts.io.DataIO
{
     ...
     public void init(Object[] ioParams)
     {
          ...
          interferogramFile = interferogramFile + ".fits";
          ...
          try
          {
              ...
              arrayWidth = prod_Ifgm.size();
              ifgmCube = new double[arrayWidth][][];
              opd = new double[arrayWidth][][];
              ...
              for(Iterator j=prod_Ifgm.keySet().iterator(); j.hasNext();)
              {
                   ...
                   arrayLength = sdiScan.size();

                   pixels_Name = new String[arrayLength];
                   ifgmCube[scan_no] = new double[arrayLength][];
                   opd[scan_no] = new double[arrayLength][];

                   int pixel_no = 0;
                   for(Iterator i=sdiScan.keySet().iterator(); i.hasNext();)
                   {
                       String pixel_Name = (String)i.next();
                       pixels_Name[pixel_no] = pixel_Name;
                       TableDataset single_Ifgm = (TableDataset)sdiScan.get(pixel_Name);


                       Numeric1dData opd_x = null;
                       Numeric1dData signal_x = null;
                       for(int iii=0; iii<single_Ifgm.getColumnCount(); iii++)
                       {
                            ...
                            if(columnName.compareToIgnoreCase("opd") == 0)
                                opd_x = (Numeric1dData)single_Ifgm.getColumn(columnName).getData();
                            if(columnName.compareToIgnoreCase("signal") == 0)
                                signal_x = (Numeric1dData)single_Ifgm.getColumn(columnName).getData();
                       }

                       if(opd_x == null) 
                       {
                            throw new java.lang.IndexOutOfBoundsException("No column available with name[opd]");
                       }
                       if(signal_x == null)
                       {
                            throw new java.lang.IndexOutOfBoundsException("No column available with name[signal]");
                       }

                       nPoints_Ifgm = signal_x.getSize();
                       opd[scan_no][pixel_no] = ((Double1d)opd_x).getArray();
                       ifgmCube[scan_no][pixel_no] = ((Double1d)signal_x).getArray();
                       pixel_no++;
                   }
                   scan_no++;
              }
